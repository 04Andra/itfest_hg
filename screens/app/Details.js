...
const moreDetails={
    ...
    "Peaches":{
        "localitate":"Timisoara",
        "descipt":"Timisoara is the capital city of Timis County, Banat, and the main economic, social and cultural centre in Western Romania.",
        "production":
    ...
    "Cow Milk":{
        ...
        "descipt":"Dumbravita is a commune in Timis County, Romania. It is composed of a single village, Dumbrăvița. One of the most developed and rich communes in Romania, Dumbrăvița has become in recent years a suburb of Timișoara.",
        "production":
            "Milk production is the process of obtaining milk from animals like cows, goats, or sheep for human consumption.",
...
function Details({navigation, route}){
    return(
        <View style={{flex:1}}>
            ...
            <View style={{borderRadius:20, backgroundColor:'#F1F5A8', height:'90%',padding:20, width:'80%',alignItems:'center'}}>
                <Text style={{fontSize:20, fontWeight:'600'}}>{route.params.prodName}</Text>
                <KSpacer h={10}/>
                <View style={{flexDirection:'column', gap:10}}>
                    <Text style={{fontSize:16,fontWeight:'600', fontStyle:'italic'}}>{moreDetails[route.params.prodName].localitate}</Text>
                    <Text style={{fontSize:16,fontWeight:'500'}}>{moreDetails[route.params.prodName].descipt}</Text>
                    <Text style={{fontSize:16,fontWeight:'600'}}>{moreDetails[route.params.prodName].producer}</Text>
                    <View style={{flexDirection:'column', gap:4}}>
                        <Text style={{fontSize:16, fontWeight:'600',fontStyle:'italic'}}>{route.params.prodName}</Text>
